...
use message_broker_pattern::Task;
use message_broker_pattern::TaskHandle;
...
///
/// The task ID is stored under [`message_broker_pattern::TASK_ID_HEADER_KEY`] so
...
fn encode_task_headers(
    ...
    map.insert(
        message_broker_pattern::TASK_ID_HEADER_KEY,
...
fn decode_task_headers(
    ...
    for (name, values) in headers.iter() {
        ...
        if (name.as_ref() as &str) == message_broker_pattern::TASK_ID_HEADER_KEY {
...
mod tests {
    ...
    fn test_decode_task_headers_malformed_id_value_returns_none_id() {
        let mut map = async_nats::HeaderMap::new();
        map.insert(message_broker_pattern::TASK_ID_HEADER_KEY, "not-a-uuid");
