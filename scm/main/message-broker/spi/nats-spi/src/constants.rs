//! NATS-specific tuning constants — this crate's own concern, not shared
//! across providers (see `message_broker_pattern::TASK_ID_HEADER_KEY` for the
//! one constant genuinely shared with `message-broker-svc-kafka-spi`).
