...
/// `KafkaConfig`'s own `Validator` impl (through `message-broker-pattern`'s
/// `Validator::validate_config`), before ever building an rdkafka client.
#[cfg(feature = "kafka")]
#[test]
...
async fn test_kafka_message_broker_health_check_fails_for_unreachable_broker() {
    use message_broker_pattern::{BrokerError, HealthCheckRequest, MessageBroker};
    use message_broker_svc_saf::MessageBrokerFactory;
...
async fn test_kafka_message_broker_publish_fails_for_unreachable_broker() {
    ...
    use message_broker_pattern::{BrokerError, Message, MessageBroker, PublishRequest};
    use message_broker_svc_saf::MessageBrokerFactory;
...
async fn test_subscribe_returns_stream_without_panicking() {
    use message_broker_pattern::{MessageBroker, SubscribeRequest};
    use message_broker_svc_saf::MessageBrokerFactory;
